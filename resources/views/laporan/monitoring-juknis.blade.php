                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-5 mb-4">
                    @foreach ($juknisResults as $juknis)
                        @php
                            ...
                            $cardColor = $isError ? 'red' : 'emerald';
                            $badgeClass = $isError ? 'badge-red' : 'badge-green';
                            $badgeText = $isError ? 'Melanggar' : 'Sesuai';
                            $iconBg = $isError ? 'bg-red-50' : 'bg-emerald-50';
                            $iconText = $isError ? 'text-red-500' : 'text-emerald-500';
                        @endphp
                        <div class="card">
                            <div class="card-header">
                                <span class="card-title">Juknis {{ $juknis['nama'] }}</span>
                                <span class="badge {{ $badgeClass }}">{{ $badgeText }}</span>
                            </div>
                            <div class="card-body space-y-3">
                                <div class="flex items-center gap-3">
                                    <div class="w-10 h-10 rounded-xl {{ $iconBg }} flex items-center justify-center flex-shrink-0">
                                        @if ($juknis['nama'] === 'Honor')
                                            <svg class="w-5 h-5 {{ $iconText }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>
                                        @elseif ($juknis['nama'] === 'Buku')
                                            <svg class="w-5 h-5 {{ $iconText }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"/></svg>
                                        @else
                                            <svg class="w-5 h-5 {{ $iconText }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"/></svg>
                                        @endif
                                    </div>
                                    <div>
                                        <div class="text-2xl font-bold text-slate-900">{{ number_format($juknis['persentase'], 2, ',', '.') }}%</div>
                                        <div class="text-[10px] text-slate-400">
                                            @if ($juknis['nama'] === 'Honor') Realisasi periode kritis
                                            @elseif ($juknis['nama'] === 'Buku') Proporsi terhadap Total Pagu
                                            @else Realisasi terhadap Total Pagu
                                            @endif
                                        </div>
                                    </div>
                                </div>

                                <div class="rounded-lg px-3 py-2 text-xs {{ $isError ? 'bg-red-50 text-red-700 border border-red-200' : 'bg-emerald-50 text-emerald-700 border border-emerald-200' }}">
                                    {{ $juknis['detail'] }}
                                </div>
                            </div>
                        </div>
