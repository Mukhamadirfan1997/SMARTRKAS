...
class RekapRekeningExport implements FromCollection, WithHeadings, WithTitle, WithMapping, WithColumnFormatting, WithStrictNullComparison
{
    protected int $bulan;
    ...
    public function map($row): array
    {
        return [
            ...
            (int) round((float) ($row->rencana_bulan ?? 0)),
            (int) round((float) ($row->realisasi_bulan ?? 0)),
            (int) round((float) ($row->sisa_bulan ?? 0)),
            ($row->persen ?? 0) . '%',
        ];
    }
